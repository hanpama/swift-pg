...
def parse_line(line) -> tuple | None:
    """
    Parse a line from the error codes file.
    """
...
def main():
    ...
    for line in lines:
        ...
        code, severity, errcode, name = parsed

        # e.g. errcode: ERRCODE_E_R_E_CONTAINING_SQL_NOT_PERMITTED
        # case_name: ereContainingSqlNotPermitted
        prefix = "ERRCODE_"
        if errcode.startswith(prefix):
            base_name = errcode[len(prefix) :]
        else:
            base_name = errcode
        ...
        case_name_parts = []
        if parts:
            case_name_parts.append(parts[0].lower())

            for part in parts[1:]:
                if part:
                    if len(part) == 1:
                        case_name_parts.append(part.lower())
                    else:
                        case_name_parts.append(part.capitalize())

        case_name = "".join(case_name_parts)

        parsed_codes.append((code, case_name))

    with open("Sources/SwiftPG/Errors/DatabaseError.swift", "w") as f:
        f.write("// This file is generated from Tools/errcodes_gen.py\n")
        f.write("// Do not edit this file directly.\n")

        f.write("public enum DatabaseError: Error {\n")

        for code, case_name in parsed_codes:
            f.write(f"  case {case_name}(details: DatabaseErrorDetails)\n")
        f.write("  /// Unknown error code\n")
        f.write("  case unknown(details: DatabaseErrorDetails)\n")
        f.write("  init(details: DatabaseErrorDetails) {\n")
        f.write("    switch details.code {\n")
        for code, case_name in parsed_codes:
            f.write(f'    case "{code}": self = .{case_name}(details: details)\n')
        f.write("    default: self = .unknown(details: details)\n")
        f.write("    }\n")
        f.write("  }\n")
        f.write("}\n")
        f.write(details)
